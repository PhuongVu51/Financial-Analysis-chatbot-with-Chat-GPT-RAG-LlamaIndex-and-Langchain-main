...
if up_files:
    all_docs = []
    with st.spinner("🤖 Đang quét OCR chất lượng cao..."):
        for f in up_files:
            path = f"temp_{f.name}"
            with open(path, "wb") as tmp: tmp.write(f.getbuffer())
            # OCR 400 DPI để nhận diện chính xác số liệu bảng biểu
    ...
    # 2. KHỞI TẠO CÔNG CỤ & AGENT (Bản sửa lỗi prompt)
    ...
    # Thiết lập System Prompt ngay trong lúc khởi tạo để tránh lỗi AttributeError
    system_prompt = (
        "Bạn là chuyên gia tài chính. Hãy dùng công cụ 'scanner' để tra cứu số liệu. "
        "Luôn phân biệt cột 'Hợp nhất' và 'Riêng lẻ'. Trả lời số liệu chính xác từ văn bản."
    )
    agent = OpenAIAgent.from_tools(tools=[tool], system_prompt=system_prompt, verbose=True)
